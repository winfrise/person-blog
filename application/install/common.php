...
function install_sql(){
	...
	if(!$mysql_link)
	{
		showProgress(array('isError' => true,'message' => 'mysql链接失败'));
	}else{
	...
	if(!@db_select($db_name,$mysql_link))
	{
		...
		{
	    	$DATABASESQL = "DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci";
		}
...
EOF;
	...
	//插入管理员数据
	$adminSql = 'insert into `'.$db_pre.'admin` (`username`,`password`) values ("'.$admin_user.'","'. Lemon_get_pwd($admin_pwd).'")';
	if(!db_query($adminSql,$mysql_link))
	{
		showProgress(array('isError' => true,'message' => '创建管理员失败'.$adminSql,'percent' => 99));
	}else{
	...
	if(!db_query($adminSql,$mysql_link))
	{
		showProgress(array('isError' => true,'message' => '更新站点配置失败'.$adminSql,'percent' => 99));
	}else{
