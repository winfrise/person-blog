...
class Download extends Model {
    ...
    protected static function init()
    {
        ...
        //编辑后可用
        Download::event('after_update', function () {
            Cache::tag('download_data')->clear();
        });
        //新增后可用
        Download::event('after_insert', function () {
            Cache::tag('download_data')->clear();
        });
        //写入后  貌似没用到
        Download::event('after_write', function () {
            Cache::tag('download_data')->clear();
        });
        //删除后
        Download::event('after_delete', function () {
            Cache::tag('download_data')->clear();
        });
    }
     ...
     * 获取下载数据库所有数据并分页
     */
    public function adminGetDownloadAllDataToPage($param){
        ...
        if(!empty($param)){
            ...
            if(isset($param['category_id']) && $param['category_id'] != 0){
                $where['d.category_id']=['eq',$param['category_id']];
            }
            //搜索条件
            if(!empty($param['search'])){
                $where['d.title']=['like',"%".$param['search']."%"];
            }
            isset($param['page_size']) && $param['page_size'] != 0 ? $page_size=(int)$param['page_size'] : '';

        }
        $field=('d.*,c.name as category_name');
    ...
    public function add($params){
        ...
        $id=$this->id;
        Url::root('/');
        $url=url('index/download/show',['id'=>$id]);
    ...
    public function edit($params){
        //Url::root('/');
        //$params['url']=url('index/download/lists',['id'=>$params['id']]);
