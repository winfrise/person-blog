...
class Page extends Model {
    ...
    protected static function init()
    {
        ...
        //编辑后可用
        Page::event('after_update', function () {
            Cache::tag('page_data')->clear();
        });
        //新增后可用
        Page::event('after_insert', function () {
            Cache::tag('page_data')->clear();
        });
        //写入后  貌似没用到
        Page::event('after_write', function () {
            Cache::tag('page_data')->clear();
        });
        //删除后
        Page::event('after_delete', function () {
            Cache::tag('page_data')->clear();
        });
    }
     ...
     * 获取单页数据库所有数据并分页
     */
    public function adminGetPageAllDataToPage($param){
     ...
     *  通过category_id获取单条信息
     */
    public function adminFindDataToCategoryId($category_id){
        $data=$this->where(['delete_time'=>0,'category_id'=>$category_id])->find();
        if(empty($data)){
            return false;
        }
        return $data->toArray();
    ...
    public function edit($params){
        Url::root('/');
        $params['url']=url('index/page/index',['id'=>$params['id']]);
