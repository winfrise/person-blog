...
class Tree
{
     ...
     * @param string $str 第一种HTML代码方式
     * @param string $str2 第二种HTML代码方式
     * @param int $banId 如果为0 所有的可选 不为0 则禁止掉对应的model_id选项   LemonCMS专用
     * @param integer $sid 默认选中
     * @param integer $adds 前缀
     */
    public function getTreeCategoryTwo($myId, $str, $str2, $banId=[], $sid = 0, $adds = '')
    {
        $number = 1;
        $child  = $this->getChild($myId);
        if (is_array($child)) {
            ...
            foreach ($child as $id => $a) {
                ...
                $this->ret .= $nstr;
                $this->getTreeCategoryTwo($id, $str, $str2, $banId ,$sid, $adds . $k . '&nbsp;');
                $number++;
            }
        }
        return $this->ret;
